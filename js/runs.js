...
const Runs = {
  ...
  add(run) {
    // strip base64 images before saving — store URL or null only
    if (run.stravaImg && run.stravaImg.startsWith('data:')) {
      run.stravaImg = null; // image upload failed or not configured — drop it
    }
    const runs = this.all();
    runs.unshift(run);
    this.save(runs);
    ...
    return `<div class="run-card${r.verified ? ' verified' : ''}" data-id="${r.id}">
      <div class="rc-head">
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap;align-items:center">
          ${verBadge}${tags}
          <button class="btn btn-ghost btn-xs" onclick="event.stopPropagation();toggleRunPublic(${r.id})"
            style="font-size:10px;padding:1px 6px">${r.isPublic !== false ? '🌍' : '🔒'}</button>
        </div>
      </div>
      <div class="rc-stats">
  ...
  loadSettings() {
    ...
    // sync status
    const syncEnabled = Auth.syncEnabled();
    const notCfg = document.getElementById('sync-not-configured');
    const isCfg  = document.getElementById('sync-configured');
    if (notCfg) notCfg.style.display = syncEnabled ? 'none' : 'block';
    if (isCfg)  isCfg.style.display  = syncEnabled ? 'block' : 'none';
    const u = Auth.current?.username;
    const lastSync = u ? localStorage.getItem(`rt_${u}_last_sync`) : null;
    const lastEl = document.getElementById('settings-last-sync');
    if (lastEl) lastEl.textContent = lastSync ? `Last synced: ${new Date(lastSync).toLocaleString()}` : 'Never synced';
    // leaderboard sharing toggle
    const lbSharing = DB.get('lb_share', false);
    const lbTog = document.getElementById('settings-lb-toggle');
    if (lbTog) lbTog.classList.toggle('on', lbSharing);
    const dataUp = DB.get('data_upload', false);
    const dataTog = document.getElementById('settings-data-upload-toggle');
    if (dataTog) dataTog.classList.toggle('on', dataUp);
  },
};
...
function exportData() {
  // Export as SQL (the secure portable format)
  Auth.exportSQL();
}
...
function handleImageUpload(e) {
  ...
  reader.onload = async ev => {
    ...
    previewEl.innerHTML = `
      <img src="${compressed}" style="max-width:100%;border-radius:6px;margin-top:8px">
      <div id="ocr-status" style="font-size:12px;color:var(--muted2);margin-top:8px;display:flex;align-items:center;gap:8px">
        <span class="spin" style="font-size:16px">⟳</span> Reading screenshot…
      </div>`;
    ...
    try {
      ...
      // ── OCR: get run data + location ──
      const ocrRes = await fetch(`${SYNC_WORKER_URL}/ocr`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ b64, mimeType:'image/jpeg', groqKey }),
      });
      if (!ocrRes.ok) throw new Error('OCR ' + ocrRes.status);
      const ocrData = await ocrRes.json();
      const parsed  = JSON.parse(ocrData.content?.[0]?.text || 'null');
      if (!parsed?.is_strava) {
        previewEl.innerHTML = `<div style="color:var(--red);font-size:12px;margin-top:8px">✗ Not a Strava screenshot.</div>`;
        document.getElementById('upload-zone').style.borderColor = '';
        return;
      }

      // ── Route detection ──
      statusEl().innerHTML = '<span class="spin" style="font-size:16px">⟳</span> Detecting route…';
      let routeWaypoints = [];
      try {
        const rtRes = await fetch(`${SYNC_WORKER_URL}/route-ocr`, {
          method:'POST', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ b64, mimeType:'image/jpeg', groqKey }),
        });
        if (rtRes.ok) routeWaypoints = (await rtRes.json()).waypoints || [];
      } catch {}

      // ── Upload image if public ──
      statusEl().innerHTML = '<span class="spin" style="font-size:16px">⟳</span> Saving…';
      const runId  = Date.now();
      const isPublic = true; // default public, user can change after

      let imgUrl = null;
      if (isPublic) {
        try {
          const upRes = await fetch(`${SYNC_WORKER_URL}/upload`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ b64, username: Auth.current.username, runId }),
          });
          if (upRes.ok) imgUrl = (await upRes.json()).url;
        } catch {}
      }
      ...
      const run = {
        id: runId,
        userId: Auth.current.username, displayName: Auth.current.displayName,
        ...
        tags, route: routeWaypoints,
        stravaImg: imgUrl,
        verified: true,
        isPublic,
        city: parsed.city || null,
        mapCenter: parsed.map_center_lat ? [parsed.map_center_lat, parsed.map_center_lng] : null,
      };
      ...
      document.getElementById('upload-zone').style.borderColor = '';
      statusEl().innerHTML = `<span style="color:var(--green)">✓ Saved: <b>${run.name}</b> — ${dist.toFixed(2)}km</span>`;

      // show route confirmation if we got waypoints
      setTimeout(() => {
        previewEl.innerHTML = '';
        if (routeWaypoints.length >= 2) {
          showRouteConfirm(run);
        } else {
          showPage('history');
        }
      }, 1200);
      ...
      const msg = err.message === 'no-worker' ? 'Sync worker not configured.'
                : err.message === 'no-groq'   ? 'Add a Groq key in Settings.'
                : 'Could not read screenshot — fill in manually.';
      if (statusEl()) statusEl().innerHTML = `<span style="color:var(--muted2)">${msg}</span>`;
    }
  };
