...
const Runs = {
  ...
  loadSettings() {
    ...
    // sync settings
    const syncTok = Auth.getSyncToken();
    const gistId = localStorage.getItem(`rt_${Auth.current?.username}_gist_id`) || '';
    const lastSync = localStorage.getItem(`rt_${Auth.current?.username}_last_sync`) || '';
    const tokEl = document.getElementById('settings-sync-token');
    const gistEl = document.getElementById('settings-gist-id');
    const lastEl = document.getElementById('settings-last-sync');
    if (tokEl) tokEl.value = syncTok;
