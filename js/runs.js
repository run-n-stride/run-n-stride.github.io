  ...
  reader.onload = async ev => {
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: 'text', text: `Look at this image. Is this a Strava activity screenshot? Respond ONLY with valid JSON, no markdown, no extra text. Fields: is_strava (boolean), distance_km (number e.g. 2.73), time_seconds (integer total seconds, e.g. 35m45s=2145 or 1h02m30s=3750), elevation_m (integer or null), name (string or null), date (YYYY-MM-DD or null). If not Strava: is_strava=false, rest null.` }
            ]
          }]
        })
      });
      ...
      const run = {
        ...
        timeSec: parsed.time_seconds || 0,
        elev: parsed.elevation_m || 0,
        pace: Runs.calcPace(parsed.distance_km || 0, parsed.time_seconds || 0),
