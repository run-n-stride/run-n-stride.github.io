...
const Leaderboard = {
  ...
  async publishRun(run) {
    ...
    try {
      await fetch(`${SYNC_WORKER_URL}/leaderboard`, {
        method: 'PUT',
  ...
  async renderGlobal() {
    ...
    let entries = [];
    try {
      const r = await fetch(`${SYNC_WORKER_URL}/leaderboard`);
  ...
  async renderRouteLB(routeId, routeName, containerEl) {
    ...
    let entries = [];
    try {
      const r = await fetch(`${SYNC_WORKER_URL}/route-lb?routeId=${routeId}`);
      if (r.ok) entries = await r.json();
  ...
  async submitToRouteLB(run, routeId) {
    if (!Auth.syncEnabled()) return;
    try {
      await fetch(`${SYNC_WORKER_URL}/route-lb`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
