...
const Leaderboard = {
  ...
  _lbEnabled() {
    // returns true if user has opted in to global leaderboard sharing
    return DB.get('lb_share', false) === true;
  },

  // Called when a run is saved — publish to global board only if opted in
  publishRun(run) {
    if (!this._lbEnabled()) return;
    const entry = {
      id: run.id,
  ...
  // Toggle sharing and retroactively publish/remove all runs
  setSharing(enabled) {
    DB.set('lb_share', enabled);
    if (enabled) {
      // retroactively publish all runs
      Runs.all().forEach(r => this.publishRun(r));
    } else {
      // remove all my entries from global board
      const global = DB.gget('runs', []);
      DB.gset('runs', global.filter(e => e.userId !== Auth.current.username));
    }
  },
  ...
  renderLocal() {
    ...
    el.innerHTML = `
      ...
      <div class="section lb-share-section">
        <h3>Leaderboard sharing</h3>
        <div class="toggle-row" style="margin-bottom:8px">
          <div>
            <span style="font-size:13px;font-weight:600">Share my runs on the global board</span>
            <p class="field-note" style="margin-top:3px">Your username and run stats will appear in the Global tab for other users on this device/browser to see.</p>
          </div>
          <div class="toggle${sharing ? ' on' : ''}" id="lb-share-tog" onclick="Leaderboard.toggleSharing()"></div>
        </div>
        ${sharing
          ? `<p class="field-note" style="color:var(--amber)">⚠ Note: If this app is hosted on GitHub Pages, data already published there is stored in shared localStorage accessible to anyone using that GitHub Pages URL. Disabling sharing here stops new data being added, but existing data on the GitHub Pages deployment cannot be removed from other users' browsers.</p>`
          : `<p class="field-note" style="color:var(--muted2)">Your runs are private and not shared. Enable to appear on the global leaderboard.</p>`
        }
      </div>`;
  },

  toggleSharing() {
    const current = this._lbEnabled();
    const next = !current;
    if (next) {
      if (!confirm('Enable global leaderboard sharing?\n\nYour username and run stats will be visible to others using this app.\n\nNote: If this site is hosted on GitHub Pages, data is shared via localStorage and cannot be fully retracted from other users who have already loaded the page.')) return;
    }
    this.setSharing(next);
    const tog = document.getElementById('lb-share-tog');
    if (tog) tog.classList.toggle('on', next);
    // re-render to update notice
    this.renderLocal();
  },
  ...
  renderGlobal() {
    ...
    if (!sorted.length) {
      el.innerHTML = '<div style="text-align:center;padding:30px;color:var(--muted2);font-size:13px">No global data yet. Enable sharing in My Stats to appear here!</div>';
      return;
    }
