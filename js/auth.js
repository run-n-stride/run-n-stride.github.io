...
`;
  ...
  _wurl() { return (SYNC_WORKER_URL || '').replace(/\/+$/, ''); },
  ...
  async pushSync() {
    ...
    try {
      const res  = await fetch(`${this._wurl()}/sync`, {
        method: 'PUT',
...
function showSyncPrompt(username) {
  ...
  // auto-pull immediately — no button needed
  setTimeout(doSyncPull, 400);
}

// ── sync prompt — fires automatically, no user action needed ──
async function doSyncPull() {
  if (!Auth.syncEnabled()) { dismissSyncPrompt(); return; }
  ...
  if (r.err) {
    errEl.textContent = 'Sync error: ' + r.err + ' — starting fresh.';
    setTimeout(dismissSyncPrompt, 2000);
    return;
  }
  dismissSyncPrompt();
}
